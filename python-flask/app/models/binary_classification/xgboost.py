...
import joblib
import pandas as pd
from app.models.binary_classification.utils import cal_metrics, load_dataset, data_preprocess, cal_feature_importance
import numpy as np
...
class TrainingProgressCallback(TrainingCallback):
    ...
    def after_iteration(self, model, epoch, evals_log):
        ...
        self.metric_data['trainLoss'].append(evals_log['train']['logloss'][-1])
        self.metric_data['validLoss'].append(evals_log['valid']['logloss'][-1])
...
class Model:
    ...
    def __init__(self, model=None, app=None, socketio=None):
        ...
        self.app = app
        self.socketio = socketio
        self.default_params = Config.DEFAULT_PARAMS['binary_classification']['xgboost'].copy()
        self.default_params.update(Config.DEFAULT_PARAMS_UNDER['binary_classification']['xgboost'])
        self.metric_data = {
            'modelName': 'xgboost',
            'epoch': [],
            'trainLoss': [],
    ...
    def train(self, dataset_path, label, custom_params={}):
        ...
        train_metrics = cal_metrics(
            np.array(train_y),
            np.array(list(1 if i > 0.5 else 0 for i in self.model.predict(dtrain))),
            np.array(self.model.predict(dtrain)),
            type='train')
        valid_metrics = cal_metrics(
            np.array(valid_y),
            np.array(list(1 if i > 0.5 else 0 for i in self.model.predict(dvalid))),
